...
import modelData from './EmployeeSavedPostModel'

const moduleName = 'Việc làm quan tâm'
...
const getSavedPostList = async () => {
  ...
  const res = await PostApi.getPostUserSavedList(filter)
...
const deletePost = async (rowData) => {
  ElMessageBox.confirm('Bạn có chắc muốn xóa tin tuyển dụng này ?', 'Cảnh báo', {
    ...
    .then(async () => {
      const res = await PostApi.removeSavePost(rowData.id)
      if (res.status === 200) {
        ElNotification({
          title: 'Success',
